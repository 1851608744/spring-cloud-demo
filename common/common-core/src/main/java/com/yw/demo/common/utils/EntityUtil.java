...
//    private static final Logger LOGGER = LoggerFactory.getLogger(EntityUtil.class);
    ...
    private final String mapper_path = "D:\\work Space\\自动生成MyBatis文件";

    private final String xml_path = "D:\\work Space\\自动生成MyBatis文件";
    /**
     **********************************使用前必读*******************
     **
     ** 填入数据库配置信息
    ...
    private final String url = "jdbc:mysql://localhost:3306/demo_user?allowMultiQueries=true&characterEncoding=utf8&useSSL=true&zeroDateTimeBehavior=convertToNull";
    ...
    private List<String> getTables() throws SQLException {
        ...
        while ( results.next() ) {
            String tableName = results.getString(1);
            										/** 此处填写   数据库表名称   的统一开头信息   可分表多次运行**/
            if ( tableName.startsWith("sys_user") ) {
    ...
    //此处定义去掉表名的“_”符号   例： cms_contract_template
  ...
  //此处定义去掉表内字段的“_”符号   例： cms_contract_ID
...
//        bw.newLine();
    ...
    public static void main( String[] args ) {
        try {
            ...
            // 自动打开生成文件的目录
            Runtime.getRuntime().exec("cmd /c start explorer D:\\work Space\\自动生成MyBatis文件");
